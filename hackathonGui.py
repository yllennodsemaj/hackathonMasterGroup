...
app.addLabel("total", "total", 5, 2)


def updateBtn(btn):
    app.setLabel("JP", dictTranslate[app.getOptionBox("food")])
    app.setLabel("price", getPrice(app.getOptionBox("food")))

def cartBtn(btn):
    total += int(app.getLabel("price"))
    app.setTextArea("cartTxt", app.getOptionBox("food") + "\n")
    app.setLabel("total", str(total))

app.addButton("cart", cartBtn, 4, 2)
app.addButton("update", updateBtn, 4, 1)
